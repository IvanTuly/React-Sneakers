...
function App() {
  ...
  React.useEffect(() => {
    ...
    async function fetchData() {
      try {
        ...
        const [cartResponse, favoritesResponse, itemsResponse, offerResponse] =
          await Promise.all([
            axios.get(
              "https://react-sneakers-5eab4-default-rtdb.europe-west1.firebasedatabase.app/cart.json"
            ),
            axios.get(
              "https://react-sneakers-5eab4-default-rtdb.europe-west1.firebasedatabase.app/favorites.json"
            ),
            axios.get(
              "https://react-sneakers-5eab4-default-rtdb.europe-west1.firebasedatabase.app/items.json"
            ),
            axios.get(
              "https://react-sneakers-5eab4-default-rtdb.europe-west1.firebasedatabase.app/offers.json"
            ),
          ]);
        ...
        cartResponse.data === null
          ? setCartItems([])
          : setCartItems(Object.values(cartResponse.data));

        favoritesResponse.data === null
          ? setFavorites([])
          : setFavorites(Object.values(favoritesResponse.data));

        offerResponse.data === null
          ? setOffers([])
          : setOffers(Object.values(offerResponse.data));
  ...
  const onAddToCart = (obj) => {
    ...
    try {
      if (
        cartItems.find((item) => Number(item.parentId) === Number(obj.parentId))
      ) {
        setCartItems((prev) =>
          prev.filter((item) => Number(item.parentId) !== Number(obj.parentId))
        );
        axios.delete(
          `https://react-sneakers-5eab4-default-rtdb.europe-west1.firebasedatabase.app/cart/key_${obj.parentId}.json`
        );
      } else {
  ...
  const onRemoveCartItem = (parentId) => {
    try {
      ...
      setCartItems((prev) =>
        prev.filter((item) => Number(item.parentId) !== Number(parentId))
      );
      //через библиотеку axios отправляем delete запрос
      axios.delete(
        `https://react-sneakers-5eab4-default-rtdb.europe-west1.firebasedatabase.app/cart/key_${parentId}.json`
      );
    } catch (error) {
  ...
  const onAddToFavorite = async (obj) => {
    try {
      ...
      if (favorites.find((item) => Number(item.id) === Number(obj.id))) {
        axios.delete(
          `https://react-sneakers-5eab4-default-rtdb.europe-west1.firebasedatabase.app/favorites/key_${obj.id}.json`
        );
  ...
  return (
    ...
    >
      <div
        className={
          cartOpened ? "wrapper clear overflowHidden" : " wrapper clear"
        }
      >
        <Drawer
          items={cartItems}
